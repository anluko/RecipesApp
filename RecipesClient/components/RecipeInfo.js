...
import { Text, View, Image, StyleSheet, Platform, Linking, TouchableWithoutFeedback, Keyboard, ScrollView } from 'react-native';
...
export default function FullInfo({ route }) {
  ...
  const fetchData = async () => {
    try {
      const ingredientsResponse =  await axios.get('http://192.168.1.7:8080/getIngredients');
      const recipeIngredientsResponse =  await axios.get('http://192.168.1.7:8080/getRecipeIngredients');
      ...
      const filterIngredients = ingredientsResponse.data.filter(ingredient => recipeIngredientsIds.includes(ingredient.Id));
        ...
        <View style = { styles.main }>
          ...
          <View  style = { styles.recipeInfoView } >
              <View style = { styles.recipeCaloriesView }>
                ...
                <Text style = {styles.recipeInfoValue}>{route.params.Calories.substring(0, 4)}  кКал</Text>
              </View>
              <View style = { styles.recipeTotalTimeView }>
                <Text style = {styles.recipeInfoText}>Время</Text>
                <Text style = {styles.recipeInfoValue}>{route.params.TotalTime}  мин</Text>
              </View>
              <View style = { styles.recipeTotalWeightView }>
                <Text style = {styles.recipeInfoText}>Вес</Text>
                <Text style = {styles.recipeInfoValue}>{route.params.TotalWeight.substring(0, 4)}  г</Text>
              </View>
              <View style = { styles.recipeTotalCO2View }>
                <Text style = {styles.recipeInfoText}>CO2</Text>
                <Text style = {styles.recipeInfoValue}>{route.params.TotalCO2Emissions.substring(0, 5)}  мг</Text>
              </View>
          </View>

          <Text style = { styles.informationText }>Источник</Text>
          <Text style = { styles.recipeSourceText }  onPress = { () => Linking.openURL(route.params.SourceUrl) }>{route.params.Source}</Text>
